...
class Pipeline(object):
    ...
    def __init__(self, nodes:List[Node]=None) -> None:
        ...
        for n in self.nodes:
            self._check_node_inputs_and_outputs(n)
    ...
    def _check_node_inputs_and_outputs(self, node):
        """Logs a warning if the inputs and outputs are the same. It's not recommended."""
        for i in node.inputs:
            if i in node.outputs:
                self._logger.warning(f"{node.name} input {i} is declare in node outputs and will be overwritten.")
        ...
        for o in node.outputs:
            for n in self.nodes:
                if n == node:
                    ...
                    self._logger.warning(f"{node.name} output {o} is also the output of {n.name} and will be overwritten.")                    
